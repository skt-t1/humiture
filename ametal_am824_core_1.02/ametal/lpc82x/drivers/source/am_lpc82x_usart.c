 ...
 * \brief 串口模式（查询或中断）设置
 */
int __usart_mode_set (am_lpc82x_usart_dev_t *p_dev, uint32_t new_mode);

/** 
 * \brief 串口硬件设置
 */
int __usart_opt_set (am_lpc82x_usart_dev_t *p_dev, uint32_t opts);

/** 
 * \brief 流控接受器流控状态设置
 */
int __usart_flow_rxstat_set (am_lpc82x_usart_dev_t *p_dev, uint32_t ctrl);

/** 
 * \brief 流控模式设置
 */
int __usart_flow_mode_set (am_lpc82x_usart_dev_t *p_dev, uint32_t mode);

/** 
 * \brief 流控发送器流控状态获取
 */
int __usart_flow_txstat_get (am_lpc82x_usart_dev_t *p_dev);
...
static int __usart_ioctl (void *p_drv, int request, void *p_arg)
{
    ...
    am_lpc82x_usart_dev_t   *p_dev      = NULL;
    amhw_lpc82x_usart_t     *p_hw_usart = NULL;
  
    if (NULL == p_drv) {
        return -AM_EINVAL;
    }
    
    p_dev      = (am_lpc82x_usart_dev_t *)p_drv;
    p_hw_usart = (amhw_lpc82x_usart_t *)p_dev->p_devinfo->usart_regbase;
    ...
    case AM_UART_BAUD_SET:
        ...
        /* 等待串口发送空闲 */
        while ((amhw_lpc82x_usart_stat_get(p_hw_usart) &
                AMHW_LPC82X_USART_STAT_TXIDLE) == 0);
    
        /* 设置串口波特率 */
        status = amhw_lpc82x_usart_baudrate_set(
                                       p_hw_usart,
...
static int __usart_tx_startup (void *p_drv)
{
    ...
    /* 使能发送中断 */
    amhw_lpc82x_usart_int_enable(p_hw_usart, AMHW_LPC82X_USART_INT_TXRDY);
...
static int __usart_poll_putchar (void *p_drv, char outchar)
{
    ...
    /* 发送非就绪或软流控模式下流控关状态 */
    if ((amhw_lpc82x_usart_stat_get(p_hw_usart) & 
         AMHW_LPC82X_USART_STAT_TXRDY) == 0 ||
        ...
        /* 发送一个数据 */
        amhw_lpc82x_usart_txdata_write(p_hw_usart, outchar);
    }
...
static int __usart_poll_getchar (void *p_drv, char *p_char)
{
    ...
    /* 接收未就绪 */
    if ((amhw_lpc82x_usart_stat_get(p_hw_usart) &
         AMHW_LPC82X_USART_STAT_RXRDY) == 0) {
        return (-AM_EAGAIN);
    } else {
        *p_inchar = amhw_lpc82x_usart_rxdata_read(p_hw_usart);
        
        /* 如果是软流控模式，检查接收的数据是否是XON/XOFF控制信号 */
        if (AM_UART_FLOWCTL_SW == p_dev->flowctl_mode) {
            if (*p_inchar == AM_LPC82X_UART_XON) {
                p_dev->flowctl_tx_stat = AM_UART_FLOWSTAT_ON;
            } else if (*p_inchar == AM_LPC82X_UART_XOFF) {
...
int __usart_mode_set (am_lpc82x_usart_dev_t *p_dev, uint32_t new_mode)
{
    ...
    if (new_mode == AM_UART_MODE_INT) {
       ...
       /* 使能接收中断 */
       amhw_lpc82x_usart_int_enable(p_hw_usart, AMHW_LPC82X_USART_INT_RXRDY);
    } else {
        
       /* 禁能所有中断 */
       amhw_lpc82x_usart_int_disable(p_hw_usart, AMHW_LPC82X_USART_INT_ALL);
    }
 ...
 * \brief usart 硬件设置
 */
int __usart_opt_set (am_lpc82x_usart_dev_t *p_dev, uint32_t options)
{
    amhw_lpc82x_usart_t *p_hw_usart;
    uint32_t      cfg_flags;
    ...
    case AM_UART_CS7:
        ...
        AM_BIT_SET_MASK(cfg_flags, AMHW_LPC82X_USART_CFG_7BIT);
        break;
        
    case AM_UART_CS8:
        AM_BIT_CLR_MASK(cfg_flags, AM_SBF(0x03, 2));
        AM_BIT_SET_MASK(cfg_flags, AMHW_LPC82X_USART_CFG_8BIT);
        break;
    ...
    if (options & AM_UART_STOPB) {
        ...
        AM_BIT_SET_MASK(cfg_flags, AMHW_LPC82X_USART_CFG_STOP_2);
    } else {
        AM_BIT_CLR_MASK(cfg_flags, AM_BIT(6));
        AM_BIT_SET_MASK(cfg_flags, AMHW_LPC82X_USART_CFG_STOP_1);
    }
    ...
    if (options & AM_UART_PARENB) {
        ...
        if (options & AM_UART_PARODD) {    
            AM_BIT_SET_MASK(cfg_flags, AMHW_LPC82X_USART_CFG_PARITY_ODD);
        } else {   
            AM_BIT_SET_MASK(cfg_flags, AMHW_LPC82X_USART_CFG_PARITY_EVEN);
        }
    } else {
        AM_BIT_CLR_MASK(cfg_flags, AM_SBF(0x03, 4));
        AM_BIT_SET_MASK(cfg_flags, AMHW_LPC82X_USART_CFG_PARITY_NO);
    }

    if ((p_hw_usart->cfg & AMHW_LPC82X_USART_CFG_ENABLE) == 1) {
        /* 等待空闲 */
        while((amhw_lpc82x_usart_stat_get(p_hw_usart) & 
               AMHW_LPC82X_USART_STAT_TXIDLE) == 0);
        amhw_lpc82x_usart_disable(p_hw_usart);
    }
    
    p_hw_usart->cfg = cfg_flags;
    amhw_lpc82x_usart_enable(p_hw_usart);
...
int __usart_flow_rxstat_set (am_lpc82x_usart_dev_t *p_dev, uint32_t ctrl)
{
    ...
    /* 如果是硬件流控，配置RTS输出引脚电位 */
        ...
        if (AM_UART_FLOWSTAT_ON == ctrl) {
            /* 拉低RTS管脚,开流     */
            amhw_lpc82x_usart_cfg_clr(p_hw_usart, 
                                      AMHW_LPC82X_USART_CFG_OEPOLHIGH);

        } else {
            /* 拉高RTS管脚,限流     */
            amhw_lpc82x_usart_cfg_set(p_hw_usart, 
                                      AMHW_LPC82X_USART_CFG_OEPOLHIGH);
        }
    } else {
        /* 如果是软件流控，发送XON/XOFF流控字符 */
        if (AM_UART_FLOWSTAT_ON == ctrl) {
            amhw_lpc82x_usart_txdata_write(p_hw_usart, AM_LPC82X_UART_XON);
        } else {
            amhw_lpc82x_usart_txdata_write(p_hw_usart, AM_LPC82X_UART_XOFF);
        } 
    }
...
int __usart_flow_mode_set(am_lpc82x_usart_dev_t *p_dev, uint32_t mode)
{
    ...
    if(AM_UART_FLOWCTL_NO == p_dev->flowctl_mode) {
        p_dev->flowctl_tx_stat = AM_UART_FLOWSTAT_ON;
        amhw_lpc82x_usart_cfg_clr(p_hw_usart, AMHW_LPC82X_USART_CFG_HWFLOW);
        amhw_lpc82x_usart_cfg_clr(p_hw_usart, AMHW_LPC82X_USART_CFG_OESEL);
    } else if (AM_UART_FLOWCTL_HW == p_dev->flowctl_mode) {

        /* 硬件流控使能     */
        amhw_lpc82x_usart_cfg_set(p_hw_usart, AMHW_LPC82X_USART_CFG_HWFLOW);
        amhw_lpc82x_usart_cfg_set(p_hw_usart, AMHW_LPC82X_USART_CFG_OESEL);
    } else {
        amhw_lpc82x_usart_cfg_clr(p_hw_usart, AMHW_LPC82X_USART_CFG_HWFLOW);
        amhw_lpc82x_usart_cfg_clr(p_hw_usart, AMHW_LPC82X_USART_CFG_OESEL);
    }
...
int __usart_flow_txstat_get (am_lpc82x_usart_dev_t *p_dev)
{
    ...
    if(AM_UART_FLOWCTL_HW == p_dev->flowctl_mode) {
        ...
        /* 硬件流控判断cts的状态     */
        if((AMHW_LPC82X_USART_STAT_CTS & 
            amhw_lpc82x_usart_stat_get(p_hw_usart)) == 0) {
...
void __usart_irq_rx_handler (am_lpc82x_usart_dev_t *p_dev)
{
    amhw_lpc82x_usart_t *p_hw_usart = NULL;
    uint32_t      int_stat   = 0;
    char data;
    ...
    p_hw_usart = (amhw_lpc82x_usart_t *)p_dev->p_devinfo->usart_regbase;
    int_stat   = amhw_lpc82x_usart_intstat_get(p_hw_usart);

    
    /* 检查中断状态寄存器是否有接收中断 */
    if ((int_stat & AMHW_LPC82X_USART_INTSTAT_RXRDY) != 0) {
        
        data = amhw_lpc82x_usart_rxdata_read(p_hw_usart);
        
        p_dev->pfn_rxchar_put(p_dev->p_rxput_arg, data);
      
        /* 如果是软件流控模式，检查接受到数据是否是XON/XOFF信号 */
        if (AM_UART_FLOWCTL_SW == p_dev->flowctl_mode) {
            if (data == AM_LPC82X_UART_XON) {
                p_dev->flowctl_tx_stat = AM_UART_FLOWSTAT_ON;
            } else if (data == AM_LPC82X_UART_XOFF) {
                p_dev->flowctl_tx_stat = AM_UART_FLOWSTAT_OFF;
            } else {
 ...
 */
void __usart_irq_tx_handler (am_lpc82x_usart_dev_t *p_dev)
{

    char     data;
    uint32_t int_stat  = NULL;
    ...
    } else if (((int_stat & AMHW_LPC82X_USART_INTSTAT_TXRDY)  != 0) ||
               ((int_stat & AMHW_LPC82X_USART_INTSTAT_TXIDLE) != 0) ) {

        if ((p_dev->pfn_txchar_get(p_dev->p_txget_arg, &data)) == AM_OK) {
            amhw_lpc82x_usart_txdata_write(p_hw_usart, data);
        } else {

            /* 数据发送完关发送就绪中断 */
            amhw_lpc82x_usart_int_disable(p_hw_usart, 
                                          AMHW_LPC82X_USART_INT_TXRDY);
        }
    } else {
    }
...
void __usart_irq_handler (void *p_arg)
{
    ...
    p_dev          = (am_lpc82x_usart_dev_t *)p_arg;
    p_hw_usart     = (amhw_lpc82x_usart_t *)p_dev->p_devinfo->usart_regbase;
    usart_int_stat = amhw_lpc82x_usart_intstat_get(p_hw_usart);
  
    if (amhw_lpc82x_usart_intstat_get (p_hw_usart) & 
        AMHW_LPC82X_USART_INTSTAT_RXRDY) {
            
        __usart_irq_rx_handler(p_dev);
    } else if (amhw_lpc82x_usart_intstat_get (p_hw_usart) & 
               AMHW_LPC82X_USART_INTSTAT_TXRDY) {
    ...
    if ((p_dev->other_int_enable & usart_int_stat) != 0) {
        ...
        if (p_dev->pfn_err != NULL) {
            p_dev->pfn_err(p_dev->p_err_arg,
                           AM_LPC82X_UART_ERRCODE_USART_OTHER_INT,
                           (void *)usart_int_stat,
                           1);
        }
    }
 ...
 * \brief usart 模块初始化
 */
am_uart_handle_t am_lpc82x_usart_init (am_lpc82x_usart_dev_t     *p_dev,
                                 const am_lpc82x_usart_devinfo_t *p_devinfo)
{
    uint32_t       tmp;

    /* 默认USART配置（8位数据，无奇偶校验，一位停止位） */
    uint32_t       usart_cfg_flags = AMHW_LPC82X_USART_CFG_8BIT      |
                                     AMHW_LPC82X_USART_CFG_PARITY_NO |
                                     AMHW_LPC82X_USART_CFG_STOP_1;
    ...
    p_hw_usart               = (amhw_lpc82x_usart_t *)p_devinfo->usart_regbase;
    ...
    if (usart_cfg_flags & AMHW_LPC82X_USART_CFG_STOP_2) {
        p_dev->options |= AM_UART_STOPB;
    }
    
    amhw_lpc82x_usart_config(p_hw_usart, usart_cfg_flags);
    amhw_lpc82x_usart_control(p_hw_usart, 0);
    
    p_dev->baud_rate = amhw_lpc82x_usart_baudrate_set(
                                    p_hw_usart,
                                    amhw_lpc82x_clk_usart_baseclkrate_get(),
                                    __USART_BAUDRATE);
...
{
    am_lpc82x_usart_dev_t *p_dev    = (am_lpc82x_usart_dev_t *)handle;
    amhw_lpc82x_usart_t *p_hw_usart = NULL;
