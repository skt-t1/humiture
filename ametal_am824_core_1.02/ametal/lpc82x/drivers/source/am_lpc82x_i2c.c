...
/** \brief I2C 速率设置函数 */
static int __i2c_speed_set (am_lpc82x_i2c_dev_t *p_dev, uint32_t bus_speed);

/** \brief I2C 硬件初始化 */
static int __i2c_hard_init (am_lpc82x_i2c_dev_t *p_dev);

/** \brief I2C 中断处理函数 */
static void __i2c_irq_handler (void *p_arg);

/** \brief I2C 状态机函数 */
static int __i2c_mst_sm_event (am_lpc82x_i2c_dev_t *p_dev, uint32_t event);
...
static int __i2c_speed_set (am_lpc82x_i2c_dev_t *p_dev, uint32_t bus_speed)
{
    amhw_lpc82x_i2c_t *p_hw_i2c  = NULL;
    ...
    p_hw_i2c  = (amhw_lpc82x_i2c_t *)p_dev->p_devinfo->i2c_regbase;
     ...
     */
    sum_div = amhw_lpc82x_clk_periph_freq_get(p_hw_i2c) / bus_speed;
    ...
    amhw_lpc82x_i2c_clkdiv_set(p_hw_i2c, f_div);
    
    p_dev->i2c_clkrate = amhw_lpc82x_clk_periph_freq_get(p_hw_i2c) / f_div;
    ...
    amhw_lpc82x_i2c_msttime_set(p_hw_i2c, scl_h, scl_l);
...
static int __i2c_slv_timeout_set (am_lpc82x_i2c_dev_t *p_dev, 
                                  uint32_t             timeout_ms)
{
    amhw_lpc82x_i2c_t *p_hw_i2c = NULL;
    uint32_t    ticks;
    ...
    /* 不需要超时 */
    if (timeout_ms == 0) {
        amhw_lpc82x_i2c_mode_disable(p_hw_i2c, AMHW_LPC82X_I2C_TIMEOUT_MODE);
        amhw_lpc82x_i2c_int_disable(p_hw_i2c, 
                                    AMHW_LPC82X_I2C_INTENSET_EVTIMEOUT |
                                    AMHW_LPC82X_I2C_INTENSET_SCLTIMEOUT);
        return AM_OK;
    }
    ...
    amhw_lpc82x_i2c_timeout_set(p_hw_i2c, ticks >> 4);
    
    amhw_lpc82x_i2c_mode_enable(p_hw_i2c, AMHW_LPC82X_I2C_TIMEOUT_MODE);
    
    amhw_lpc82x_i2c_int_enable(p_hw_i2c, AMHW_LPC82X_I2C_INTENSET_EVTIMEOUT |
                                         AMHW_LPC82X_I2C_INTENSET_SCLTIMEOUT);
    return AM_OK;
}
...
static int __i2c_hard_init(am_lpc82x_i2c_dev_t *p_dev)
{
    ...
    amhw_lpc82x_i2c_mode_disable(p_hw_i2c, AMHW_LPC82X_I2C_MASTER_MODE);

    __i2c_speed_set(p_dev, p_dev->p_devinfo->bus_speed);

    amhw_lpc82x_i2c_mode_enable(p_hw_i2c, AMHW_LPC82X_I2C_MASTER_MODE);
 ...
 */
am_static_inline
void __i2c_msg_in (am_lpc82x_i2c_dev_t *p_dev, struct am_i2c_message *p_msg)
{
 ...
 */
am_static_inline
struct am_i2c_message *__i2c_msg_out (am_lpc82x_i2c_dev_t *p_dev)
{
...
static void __i2c_irq_handler (void *p_arg)
{
    am_lpc82x_i2c_dev_t *p_dev    = NULL;
    amhw_lpc82x_i2c_t   *p_hw_i2c = NULL;
    uint32_t i2c_status;
    ...
    p_dev    = (am_lpc82x_i2c_dev_t *)p_arg;
    p_hw_i2c = (amhw_lpc82x_i2c_t *)p_dev->p_devinfo->i2c_regbase;

    i2c_status = amhw_lpc82x_i2c_intstat_get(p_hw_i2c);

    /* 总线仲裁丢失 启动/停止错误 */
    if ((i2c_status & AMHW_LPC82X_I2C_STAT_MSTARBLOSS)||        
        (i2c_status & AMHW_LPC82X_I2C_STAT_MSTSSERR)) {         

        /* 清除状态          */
        amhw_lpc82x_i2c_stat_clr(p_hw_i2c,
                          AMHW_LPC82X_I2C_STAT_MSTARBLOSS |
                          AMHW_LPC82X_I2C_STAT_MSTSSERR);

        __i2c_mst_sm_event(p_dev, __I2C_EVT_MST_BERR);
    } 

    /* 超时 */
    if  ((i2c_status & AMHW_LPC82X_I2C_STAT_EVTIMEOUT) || 
         (i2c_status & AMHW_LPC82X_I2C_STAT_SCLTIMEOUT)) {

        /* 清除状态          */
        amhw_lpc82x_i2c_stat_clr(p_hw_i2c, AMHW_LPC82X_I2C_STAT_EVTIMEOUT |
                                           AMHW_LPC82X_I2C_STAT_SCLTIMEOUT);
    ...
    /* 主机挂起状态       */
    if (i2c_status & AMHW_LPC82X_I2C_STAT_MSTPEND) {
 
        __i2c_mst_sm_event(p_dev, (amhw_lpc82x_i2c_stat_get(p_hw_i2c) & 
                                   AMHW_LPC82X_I2C_MASTER_STATE_MASK));
    }
}
...
{
    am_lpc82x_i2c_dev_t *p_dev  = (am_lpc82x_i2c_dev_t *)p_drv;
    ...
    if (p_dev->busy == TRUE) {
        ...
        /* 打开挂起、仲裁丢失、启动/停止错误中断 */
        amhw_lpc82x_i2c_int_enable(
            (amhw_lpc82x_i2c_t *)p_dev->p_devinfo->i2c_regbase, 
            AMHW_LPC82X_I2C_INTENSET_MSTPEND    |
            AMHW_LPC82X_I2C_INTENSET_MSTARBLOSS |
            AMHW_LPC82X_I2C_INTENSET_MSTSSERR);
...
void am_lpc82x_i2c_deinit (am_i2c_handle_t handle)
{
    ...
    p_dev    = (am_lpc82x_i2c_dev_t *)handle;
    p_hw_i2c = (amhw_lpc82x_i2c_t *)p_dev->p_devinfo->i2c_regbase;
    ...
    while (1) {
        ...
        case __I2C_ST_TRANS_START : 
        {
            ...
            if (__I2C_TRANS_EMPTY(p_dev)) {
                ...
                amhw_lpc82x_i2c_mst_stop(p_hw_i2c);
                
                __I2C_NEXT_STATE(__I2C_ST_MSG_START, __I2C_EVT_TRANS_LAUNCH);
                ...
                if ((p_cur_trans->flags & AM_I2C_M_NOSTART) &&
                    ...
                    bool_t is_read = (p_cur_trans->flags & AM_I2C_M_RD) ?
                                     TRUE : FALSE;

                    if (is_read == TRUE) {
                        
                        /* 如果是接收，需要自行一次continue，以便继续接收数据 */
                        if (amhw_lpc82x_i2c_stat_get(p_hw_i2c) &
                            AMHW_LPC82X_I2C_STAT_MSTPEND) {
                                
                            amhw_lpc82x_i2c_mst_continue(p_hw_i2c);
                        }
        ...
        case __I2C_ST_SEND_SLA_ADDR :                         /* 发送从机地址 */
        {
            ...
            /* 发送设备地址 */
            amhw_lpc82x_i2c_mstdat_write(p_hw_i2c, dev_addr);
            amhw_lpc82x_i2c_mst_start(p_hw_i2c);
        ...
        case __I2C_ST_M_RECV_DATA:
        {
            ...
            (p_cur_trans->p_buf)[p_dev->data_ptr++] = \
                                         amhw_lpc82x_i2c_mstdat_read(p_hw_i2c);
        ...
        case __I2C_ST_M_SEND_DATA :
        {
            ...
            if (p_dev->data_ptr < p_cur_trans->nbytes) {
                ...
                amhw_lpc82x_i2c_mstdat_write(p_hw_i2c, 
                                     (p_cur_trans->p_buf)[p_dev->data_ptr++]);
        ...
        case __I2C_ST_ARBI_LOST:
        {
            ...
            /**
             * 这种错误出现在从机地址或者第一个字节数据传输时，重新启动发送
             */
            if (p_dev->data_ptr == 0) {
                /* 返回到从机地址发送状态 */
                __I2C_NEXT_STATE(__I2C_ST_SEND_SLA_ADDR, __I2C_EVT_NONE);
