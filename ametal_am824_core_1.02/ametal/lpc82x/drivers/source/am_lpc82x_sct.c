...
void __sct_int (void *handle)
{
    const am_lpc82x_sct_devinfo_t  *p_devinfo =
                                   ((am_lpc82x_sct_handle_t)handle)->p_devinfo;
    ...
    int_flags = amhw_lpc82x_sct_evt_flag_get(p_hw_sct);
    for (evt_num = 0; evt_num < AMHW_LPC82X_SCT_EVT_NUM; evt_num++) {
        if (AM_BIT_ISSET(int_flags, evt_num)) {
            ...
            /* 清除中断 */
            amhw_lpc82x_sct_evt_flag_clr(p_hw_sct, evt_num);
        }
    }
}
 ...
 * \brief SCT事件控制计时器配置
 */
static void __sct_evt_ctrl_cnt_cfgs(amhw_lpc82x_sct_t  *p_hw_sct,
                                    uint8_t             evt_num,
                                    uint8_t             out_cnt_flags)
{
    /* 限制计数器 */
    if(AM_BIT_ISSET(out_cnt_flags, AM_LPC82X_SCT_LIMIT_BITS_START)) {
        amhw_lpc82x_sct_evt_limit_enable(p_hw_sct,
                                         AMHW_LPC82X_SCT_MODE_UNIFY, evt_num);
    } else {
        amhw_lpc82x_sct_evt_limit_disable(p_hw_sct,
                                         AMHW_LPC82X_SCT_MODE_UNIFY, evt_num);
    }
    /* 停止计数器 */
    if(AM_BIT_ISSET(out_cnt_flags, AM_LPC82X_SCT_STOP_BITS_START)) {
        amhw_lpc82x_sct_evt_stop_enable(p_hw_sct,
                                        AMHW_LPC82X_SCT_MODE_UNIFY, evt_num);
    } else {
        amhw_lpc82x_sct_evt_stop_disable(p_hw_sct,
                                         AMHW_LPC82X_SCT_MODE_UNIFY, evt_num);
    }
    /* 启动计数器 */
    if(AM_BIT_ISSET(out_cnt_flags, AM_LPC82X_SCT_START_BITS_START)) {
        amhw_lpc82x_sct_evt_start_enable(p_hw_sct,
                                         AMHW_LPC82X_SCT_MODE_UNIFY, evt_num);
    } else {
        amhw_lpc82x_sct_evt_start_disable(p_hw_sct,
                                          AMHW_LPC82X_SCT_MODE_UNIFY, evt_num);
    }
    /* 终止计数器 */
    if(AM_BIT_ISSET(out_cnt_flags, AM_LPC82X_SCT_HALT_BITS_START)) {
        amhw_lpc82x_sct_evt_halt_enable(p_hw_sct,
                                        AMHW_LPC82X_SCT_MODE_UNIFY, evt_num);
    } else {
        amhw_lpc82x_sct_evt_halt_disable(p_hw_sct,
                                         AMHW_LPC82X_SCT_MODE_UNIFY, evt_num);
    }
}
 ...
 * \brief SCT事件控制输出IO配置
 */
static void __sct_evt_ctrl_io_cfgs(amhw_lpc82x_sct_t  *p_hw_sct,
                                     uint8_t             evt_num,
                                     uint16_t            out_io_flags)
{
    ...
    /* 设置输出通道置位 */
    ch = AM_BITS_GET(out_io_flags, AM_LPC82X_SCT_IOSET_CH_BITS_START,
                                   AMHW_LPC82X_SCT_OUT_NUM);

    for (i = 0; i < AMHW_LPC82X_SCT_OUT_NUM; i++) {
        if (AM_BIT_ISSET(ch, i)) {
            amhw_lpc82x_sct_out_set_enable(p_hw_sct, i, evt_num);
        } else {
            amhw_lpc82x_sct_out_set_disable(p_hw_sct, i, evt_num);
        }
    }

    /* 设置输出通道清零 */
    ch = AM_BITS_GET(out_io_flags,
            AM_LPC82X_SCT_IOCLR_CH_BITS_START,
            AMHW_LPC82X_SCT_OUT_NUM);

    for (i = 0; i < AMHW_LPC82X_SCT_OUT_NUM; i++) {
        if (AM_BIT_ISSET(ch, i)) {
            amhw_lpc82x_sct_out_clr_enable(p_hw_sct, i, evt_num);
        } else {
            amhw_lpc82x_sct_out_clr_disable(p_hw_sct, i, evt_num);
        }
    }
}
 ...
 * \brief SCT事件匹配或捕获来设置
 */
static void __sct_evt_matchcap_cfgs(amhw_lpc82x_sct_t  *p_hw_sct,
                                      uint8_t      evt_num,
                                      uint64_t     cond_flags)
{
    uint32_t tmp;
    ...
    matcap_num     = AM_BITS_GET(cond_flags,
                                 AM_LPC82X_SCT_MATCAP_CH_BITS_START,
                                 AM_LPC82X_SCT_MATCAP_CH_BITS_LEN);

    tmp = AM_BIT_ISSET(cond_flags,
                       AM_LPC82X_SCT_MATCAP_ISCAP_BITS_START) ?
    ...
    amhw_lpc82x_sct_regmode_config(p_hw_sct,
                                   ...
                                   matcap_num,
                                   (amhw_lpc82x_sct_matcap_mode_t)tmp);

    /* 如果是匹配条件 */
    if (tmp == AMHW_LPC82X_SCT_MATCAP_MATCH) {
        tmp = (cond_flags >> AM_LPC82X_SCT_MAT_VAL_BITS_START) &
               0xFFFFFFFF;
 ...
 * \brief SCT事件中断配置
 */
static void __sct_evt_ctrl_int_cfg (am_lpc82x_sct_handle_t sct_handle,
                                    uint8_t                evt_num,
                                    am_pfnvoid_t           pfn_isr,
                                    void                  *p_arg,
                                    int                    slot)
{
    const am_lpc82x_sct_devinfo_t  *p_devinfo = sct_handle->p_devinfo;
    amhw_lpc82x_sct_t              *p_hw_sct  = NULL;
    ...
    if (NULL != pfn_isr) {
        ...
        amhw_lpc82x_sct_evt_int_enable(p_hw_sct, evt_num);
    } else {
        amhw_lpc82x_sct_evt_int_disable(p_hw_sct, evt_num);
 ...
 * \brief SCT事件控制DMA配置
 */
static void __sct_evt_ctrl_dma_cfgs (amhw_lpc82x_sct_t *p_hw_sct,
                                     uint8_t            evt_num,
                                     uint8_t            out_dma_flags)
{
    ...
    /* 设置输出通道清零 */
    ch = AM_BITS_GET(out_dma_flags, AM_LPC82X_SCT_DMA_REQ_CH_BITS_START,
                                    AMHW_LPC82X_SCT_DMA_REQ_NUM);

    for (i = 0; i < AMHW_LPC82X_SCT_DMA_REQ_NUM; i++) {
        if (AM_BIT_ISSET(ch, i)) {
            amhw_lpc82x_sct_dma_req_evt_enable(p_hw_sct, i, evt_num);
        } else {
            amhw_lpc82x_sct_dma_req_evt_disable(p_hw_sct, i, evt_num);
        }
    }
}
/**
                           ...
                           am_lpc82x_sct_evt_t   *p_evt)
{
    ...
    if (NULL == sct_handle             ||
        NULL == p_evt                  ||
        FALSE == sct_handle->valid_flg ||
        evt_num >= AMHW_LPC82X_SCT_EVT_NUM  ||
    ...
    p_hw_sct = (amhw_lpc82x_sct_t *)(sct_handle->p_devinfo->sct_regbase);

    /* 配置事件前，先关闭该事件 */
    amhw_lpc82x_sct_evt_state_cfg(p_hw_sct, evt_num, 0);
    ...
    /* 配置事件 */
    amhw_lpc82x_sct_evt_ctrl(p_hw_sct, evt_num, evt_ctrl_cfgs);

    /* 是否设置了捕获或者匹配事件 */
    if(AM_BIT_ISSET(p_evt->cond_flags, AM_LPC82X_SCT_MATCAP_ISSET_BITS_START)) {
    ...
    /* 事件对IO输出的控制 */
    __sct_evt_ctrl_io_cfgs(p_hw_sct, evt_num, p_evt->out_io_flags);

    /* 事件对DMA触发的控制 */
    __sct_evt_ctrl_dma_cfgs(p_hw_sct, evt_num, p_evt->out_dma_flags);

    /* 使能一个事件在某些状态中发生 */
    amhw_lpc82x_sct_evt_state_cfg(p_hw_sct, evt_num, p_evt->stat_en_flags);
...
int am_lpc82x_sct_halt (am_lpc82x_sct_handle_t sct_handle)
{
    ...
    if (NULL == sct_handle || FALSE == sct_handle->valid_flg ) {
        return -AM_EINVAL;
    }
    p_hw_sct = (amhw_lpc82x_sct_t *)(sct_handle->p_devinfo->sct_regbase);
                                       ...
                                       uint8_t                value)
{
    ...
    amhw_lpc82x_sct_conflict_res_config(p_hw_sct, out_num, value);
                             ...
                             uint16_t               state)
{
    ...
    if (!amhw_lpc82x_sct_halt_check(p_hw_sct, AMHW_LPC82X_SCT_MODE_UNIFY)) {
        return -AM_EPERM;
    }

    amhw_lpc82x_sct_state_set(p_hw_sct, AMHW_LPC82X_SCT_MODE_UNIFY, state);
                                 ...
                                 uint8_t                value)
{
    ...
    if (NULL == sct_handle ||
        FALSE == sct_handle->valid_flg ||
        out_num >= AMHW_LPC82X_SCT_OUT_NUM) {
        return -AM_EINVAL;
    }
    ...
    if (value == AM_GPIO_LEVEL_LOW) {
        amhw_lpc82x_sct_out_state_clr(p_hw_sct, out_num);
    } else {
        amhw_lpc82x_sct_out_state_set(p_hw_sct, out_num);
    }
                                     ...
                                     const am_lpc82x_sct_devinfo_t *p_devinfo)
{
    ...
    amhw_lpc82x_sct_config(p_hw_sct,
                    AMHW_LPC82X_SCT_CFG_32BIT_COUNTER  | /* 32为定时器            */
                    AMHW_LPC82X_SCT_CFG_CLKMODE_SYSCLK); /* 系统时钟              */
...
{
    am_lpc82x_sct_dev_t *p_dev    = (am_lpc82x_sct_dev_t *)handle;
    amhw_lpc82x_sct_t   *p_hw_sct = NULL;
    uint8_t              evt_num;
    ...
    p_hw_sct = (amhw_lpc82x_sct_t *)(handle->p_devinfo->sct_regbase);
    ...
    /* 关闭所有事件 */
    for (evt_num = 0; evt_num < AMHW_LPC82X_SCT_OUT_NUM; evt_num++) {
        amhw_lpc82x_sct_evt_state_cfg(p_hw_sct, evt_num, 0);
    }
    p_dev->evt_stat  = 0;               /* 所有事件未被使用  */
    p_dev->valid_flg = FALSE;           /* 参数无效 */
